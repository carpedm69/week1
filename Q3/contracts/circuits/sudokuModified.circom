...
template sudoku() {
    ...
    component rangeProof[9][9][2];

    for (var i=0; i<9; i++){
        for (var j=0; j<9; j++) {
            rangeProof[i][j][0] = RangeProof(32);
            rangeProof[i][j][0].range[0] <== 0;
            rangeProof[i][j][0].range[1] <== 9;

            rangeProof[i][j][1] = RangeProof(32);
            rangeProof[i][j][1].range[0] <== 0;
            rangeProof[i][j][1].range[1] <== 9;
        }
    }
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rangeProof[i][j][0].in <== solution[i][j];
            rangeProof[i][j][1].in <== puzzle[i][j];

            rangeProof[i][j][0].out === 1;
            rangeProof[i][j][1].out === 1;
